...
def load_flow_tasks_dependencies(flow):
    for task in flow.G.nodes:
        explicit_deps = load_flow_explicit_tasks_dependencies(flow, task)
        for dep in explicit_deps:
            flow.set_dependency(task, dep)

        implicit_deps = load_flow_implicit_tasks_dependencies(flow, task)
